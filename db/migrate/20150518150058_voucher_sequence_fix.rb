class VoucherSequenceFix < ActiveRecord::Migration
  def change
    ...
    # add_index :voucher_sequences, [:classification, :business_entity_id, :valid_from], unique: true, name: 'idx_voucher_seq_on_business_entity_n_classification_n_valid_from'
    # VoucherSequence.where(classification: 4).update_all(active: false)

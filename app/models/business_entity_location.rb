class BusinessEntityLocation < MyActiveRecord
  ...
  def valid_bank_account
    errors.add(:base, 'Invalid bank account selected') and return false if Account.where(type: 'Account::BankAccount', business_entity_id: self.business_entity_id).blank?
  end
end

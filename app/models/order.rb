class Order < ActiveRecord::Base
  ...
  # before_validation lambda { self.booked_by_id = SystemSettings.default_user_for_order_booking if self.booked_by.blank? && SystemSettings.default_user_for_order_booking.present? }
  before_validation :ensure_currency_presence

  before_save :update_amount_from_line_items

  def update_amount_from_line_items
    # self.amount = line_items.inject(0) { |result, line_item| result += line_item.amount }
